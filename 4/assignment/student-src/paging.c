...
void proc_init(pcb_t *proc) {
     ...
     */
    pfn_t basereg = free_frame();                           // find base register for new frame
    memset(mem + (basereg * PAGE_SIZE), 0, PAGE_SIZE);      // clear all previous data at this value for future use
     ...
     */
    proc->saved_ptbr = basereg;                             // record new ptbr value into proc
    frame_table[basereg].protected = 1;                     // protect this block
}
...
void context_switch(pcb_t *proc) {
    PTBR = proc->saved_ptbr;    // We should only update the PTBR because the current process is managed by the simulator
}
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    uint16_t offset = vaddr_offset(address);            // calculate offset
    vpn_t vpn = vaddr_vpn(address);                     // calculate vpn
    pte_t* entry = (pte_t*) (mem + PTBR * PAGE_SIZE);   // assign entry pointer to desired location
    entry += vpn;                                       // add on vpn value


    /* If an entry is invalid, just page fault to allocate a page for the page table. */
    if (!entry->valid) page_fault(address);             // give page fault if invalid

    /* Set the "referenced" bit to reduce the page's likelihood of eviction */
    frame_table[PTBR].referenced = 1;                   // set referenced bit to 1
    ...
    /*
        ...
        table entry.
    */
    uint8_t* p_add = (u_int8_t*) (mem + entry->pfn * PAGE_SIZE + offset); // make p_add a ptr for write case

    /* Either read or write the data to the physical address
       depending on 'rw' */
    if (rw == 'r') {
        stats.reads++;              // update reads statistic
        return *p_add;              // return address
    } else {
        stats.writes++;             // update writes statistic
        *p_add = data;              // update value at memory address
        entry->dirty = 1;           // make entry dirty due to writing
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        if (ptr->swap) swap_free(ptr);                      // free value if previously swapped
        if (ptr->valid) frame_table[ptr->pfn].mapped = 0;   // if value is valid, re-establish mapped bit to 0
        ptr++;                                              // increment to next pte
    }
