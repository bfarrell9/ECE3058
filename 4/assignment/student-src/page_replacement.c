...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pfn_t ptbr = frame_table[victim_pfn].process->saved_ptbr;   // get PTBR
        pte_t* entry = (pte_t*)(mem + (PAGE_SIZE * ptbr));          // calculate bage table base address from memory base value and PTBR
        entry += frame_table[victim_pfn].vpn;                       // add on virtual page number
        // handle case of dirty bit (writeback required)
        if (entry->dirty) {                                         
            swap_write(entry, (void*) (mem + victim_pfn * PAGE_SIZE));
            stats.writebacks++;
        }
        entry->valid = 0;                                           // clear valid bit for future use
        frame_table[victim_pfn].mapped = 0;                         // clear mapped bit
    }
    /* Return the pfn */
    return victim_pfn;
}
