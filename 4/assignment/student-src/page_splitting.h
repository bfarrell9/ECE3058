...
static inline vpn_t vaddr_vpn(vaddr_t addr) {
	//return bits of addr that contains the original bits greater than index OFFSET_LEN
	return addr / PAGE_SIZE;
}

/* Get the offset into the page from a virtual address. */
static inline uint16_t vaddr_offset(vaddr_t addr) {
	//return bits of addr that contains the original bits less than or equal to index OFFSET_LEN
	return addr % PAGE_SIZE;
}
